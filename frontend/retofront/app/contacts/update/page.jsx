...
export default function EditContact() {
    ...
    return (
        <div className="w-4/5 max-h-full mx-auto my-0 mt-10 bg-white shadow-xl rounded-lg text-gray-900 flex flex-col relative">
            {showAlert && (
                <div className="px-4 py-3 leading-normal text-green-700 bg-green-100 rounded-lg mb-7 ml-9" role="alert">
                    <p>Contact updated successfully!</p>
                </div>
            )}
            <button className="absolute top-4 left-4 bg-purple-500 text-white px-4 py-2 rounded-lg font-semibold transform transition-all hover:-translate-y-2 duration-300 shadow-lg hover:shadow-2xl" onClick={() => router.push("/dashboard")}>Back</button>
            ...
            <div className="mx-auto w-32 h-32 relative -mt-16 border-4 border-white rounded-full overflow-hidden">
                <img className="object-cover object-center h-32" src={contact.profilePictureUrl} alt='Woman looking front'/>
            </div>
                ...
                {({ errors, touched }) => (
                    <Form className="flex flex-col items-center justify-center mt-4">
                        ...
                        <label className='block font-medium mb-1'>
                           Phone:
                    <input className='border rounded px-4 py-2 w-full focus:outline-none focus:ring focus:border-purple-500' type="tel" value={contact.phoneNumber} onChange={(e) => setPhone(e.target.value)} />
                </label>
                        </div>
                        <div className="w-3/4 mb-2">
                        ...
                        <button type="submit" className="w-1/2 block mx-auto rounded-full bg-purple-500 font-semibold text-white px-6 py-2 transform transition-all hover:-translate-y-2 duration-300 shadow-lg hover:shadow-2xl">Submit</button>
                    </Form>
                )}
            </Formik>
        </div>
